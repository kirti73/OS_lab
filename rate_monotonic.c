...
void CPU_Utilisation(struct Process t[], int n) {
    double U = 0.0;
    for (int i = 0; i < n; i++)
        U += (double)t[i].bt / t[i].period;

    double bound = n * (pow(2.0, 1.0 / n) - 1.0);

    printf("\n--- CPU Utilisation ---\n");
    printf("U = Sigma(Ci/Ti)    : %.4f (%.2f%%)\n", U, U * 100.0);
    printf("Liu & Layland bound : %.4f (%.2f%%)\n", bound, bound * 100.0);

    if (U <= bound)
        printf("Task set is schedulable (U <= bound).\n");
    else if (U <= 1.0)
        printf("U exceeds Liu & Layland bound but may still be schedulable.\n");
    else
        printf("Warning: U > 1.0 -- task set exceeds processor capacity!\n");
}
// -------- Gantt Chart Printer --------
...
void EDF(struct Process p[], int n) {
    ...
    while (completed < n) {
        ...
        for (int i = 0; i < n; i++) {
            if (remaining[i] > 0 && p[i].deadline < earliest) {
...
void RMS(struct Process p[], int n) {
    ...
    while (completed < n) {
        int idx = -1;
        int shortest_period = 1e9;

        // Pick highest priority (smallest period)
        for (int i = 0; i < n; i++) {
            if (remaining[i] > 0 && p[i].period < shortest_period) {
                shortest_period = p[i].period;
                idx = i;
            }
        }
